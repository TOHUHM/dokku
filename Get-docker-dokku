...
echo -------------------------------Now begin to setup Dokku Env and Repository----------------------------------------------
sleep 5
...
finalize_yum_repo ()
{
  ...
  yum install -y pygpgme --disablerepo='dokku_dokku'
  ...
  if [ "$?" != "0" ]; then
    ...
    sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/dokku_dokku.repo
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='dokku_dokku'
  ...
  echo "Generating yum cache for dokku_dokku..."
  yum -q makecache -y --disablerepo='*' --enablerepo='dokku_dokku'

  echo "Generating yum cache for dokku_dokku-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='dokku_dokku-source'
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/dokku/dokku/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/dokku_dokku.repo
  else
    yum_repo_path=/etc/yum.repos.d/dokku_dokku.repo
  fi
...
echo --------------------------------Dokku Environment and Repository are configured-------------------------
